...
public:
  ...
  std::string toString() const {
    ...
    for (const auto &arg : getArguments())
      SS << quoteArgument(arg) << " ";
    if (hasOutputFile())
      SS << ">" << quoteArgument(getOutputFile()) << " ";
...
private:
  // COMPAT(libfuzzer-shell-argv): preserve literal argv and redirection paths
  // when the POSIX executors pass this command through /bin/sh.
  static std::string quoteArgument(const std::string &Arg) {
#if LIBFUZZER_POSIX
    std::string Quoted("'");
    for (char C : Arg) {
      if (C == '\'')
