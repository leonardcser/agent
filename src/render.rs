...
fn render_block(block: &Block, _width: usize) -> u16 {
    match block {
        ...
        Block::Confirm { tool, desc, choice } => render_confirm_result(tool, desc, choice.clone()),
        Block::Error { message } => {
            print_error(message);
            1
        }
...
fn render_confirm_result(tool: &str, desc: &str, choice: Option<ConfirmChoice>) -> u16 {
    ...
    if let Some(c) = choice {
        ...
        match c {
            ...
            ConfirmChoice::YesWithMessage(_) => {
                let _ = out
                    .queue(SetAttribute(Attribute::Dim))
                    .and_then(|o| o.queue(Print("approved\r\n")))
                    .and_then(|o| o.queue(SetAttribute(Attribute::Reset)));
...
pub fn tool_arg_summary(name: &str, args: &HashMap<String, serde_json::Value>) -> String {
    let base = match name {
        ...
        "ask_user_question" => {
            let count = args.get("questions").and_then(|v| v.as_array()).map(|a| a.len()).unwrap_or(0);
...
pub fn show_confirm(
    ...
    // Fixed rows: bar + command + blank + "Allow?" + 3 options + blank + hint + blank = 11
    ...
    // +1 for message input row when editing
...
/// Returns `None` if cancelled, `Some(json)` with answers if confirmed.
pub fn show_ask_question(questions: &[Question]) -> Option<String> {
    ...
    // Per-question state: selected option index, and toggle set for multi-select.
    let mut selections: Vec<usize> = questions.iter().map(|_| 0).collect();
    let mut multi_toggles: Vec<Vec<bool>> = questions
        .iter()
        .map(|q| vec![false; q.options.len() + 1]) // +1 for "Other"
        .collect();
    ...
    // Calculate layout
    let max_options = questions.iter().map(|q| q.options.len()).max().unwrap_or(0) + 1; // +1 for Other
    let has_tabs = questions.len() > 1;
    // bar(1) + tabs(0|1) + blank+question+blank(3) + options(max_options) + blank(1) + hint(1)
    ...
    let draw = |active_tab: usize,
        ...
        for (i, opt) in q.options.iter().enumerate() {
            ...
            // Show description for current option
            if is_current {
                let _ = out.queue(SetAttribute(Attribute::Dim));
        ...
        if editing_other[active_tab] || (is_other_current && !other_texts[active_tab].is_empty()) {
            let _ = out.queue(Print(", "));
            let _ = out.queue(Print(&other_texts[active_tab]));
            if editing_other[active_tab] {
                // Flush to get accurate cursor position
                let _ = out.flush();
        ...
        // Show real cursor when editing Other text, hide otherwise
    ...
    let cancelled = loop {
        if let Ok(Event::Key(KeyEvent { code, modifiers, .. })) = event::read() {
            ...
            match (code, modifiers) {
                ...
                (KeyCode::Enter, _) => {
                    // If on "Other" and no text yet, start editing
                    if selections[active_tab] == other_idx && other_texts[active_tab].is_empty() {
                ...
                (KeyCode::Tab, _) => {
                    // Tab on "Other" enters editing mode
                ...
                (KeyCode::BackTab, _) | (KeyCode::Left, _) | (KeyCode::Char('h'), _) => {
                    if questions.len() > 1 {
                        active_tab = if active_tab == 0 { questions.len() - 1 } else { active_tab - 1 };
                        draw(active_tab, &selections, &multi_toggles, &other_texts, &editing_other);
                    }
                }
                (KeyCode::Up, _) | (KeyCode::Char('k'), _) => {
                ...
                (KeyCode::Char(c), _) if c.is_ascii_digit() => {
                    ...
                    if num >= 1 && num <= other_idx + 1 {
                        if q.multi_select {
                            multi_toggles[active_tab][num - 1] = !multi_toggles[active_tab][num - 1];
                        } else {
    ...
    for (i, q) in questions.iter().enumerate() {
        ...
        let answer = if q.multi_select {
            ...
            if selected.is_empty() {
                // If nothing toggled, use the cursor position
